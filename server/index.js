...
//Database connection
const connectString =
  "mongodb+srv://user1:mypassword@postitcluster.l7gyu8r.mongodb.net/postITDb?appName=PostITCluster";
...
app.put("/likePost/:postId/", async (req, res) => {
  ...
  try {
    ...
    //indexOf method returns the index of the first occurrence of a specifiedvalue in an array.
    ...
    if (userIndex !== -1) {
      ...
      const udpatedPost = await PostModel.findOneAndUpdate(
        { _id: postId },
        {
          $inc: { "likes.count": -1 }, // Decrement the like count $inc and$pull are update operators$pull: { "likes.users": userId }, // Remove userId from the users array
        },
      ...
      const updatedPost = await PostModel.findOneAndUpdate(
        { _id: postId },
        {
          ...
          $addToSet: { "likes.users": userId }, // Add userId to the usersarray if not already present
        },
        { new: true } // Return the modified document
      );
